...
//      trainStopwatch.start();
      System.err.printf("Building encoding..."); System.err.flush();
...
//   System.err.printf("Done. Took %.2f seconds\n",trainStopwatch.getTotalElapsedTime());
  ...
  public static class ObjectiveFunction <F,L> implements DifferentiableFunction {
    ...
    private Pair<Double, double[]> calculate(double[] x) {
      ...
      for (EncodedDatum datum: data) {
    	  ...
    	  for(int num=0; num < numActiveFeatures; ++num) {
    		  int featureIndex = datum.getFeatureIndex(num);     	
    		  int correctLinearFeatureIndex = indexLinearizer.getLinearIndex(featureIndex, correctLabelIndex);
    		  double featureCount = datum.getFeatureCount(num);
...
//		  oos.writeObject(indexLinearizer);		  
//	  } catch (IOException e) {
//
//		  e.printStackTrace();
//	  }
...
//		  this.encoding = (Encoding<F,L>) ois.readObject();
//		  this.indexLinearizer = (IndexLinearizer) ois.readObject();
//	  } catch (Exception e) {
//		  e.printStackTrace();
//	  }
//  }
